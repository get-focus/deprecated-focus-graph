...
const fieldForListBuilder = (entityPathList, propertyNameList, multiple= false, textOnly= false) => {
    const fieldForLineBuilder = (connectedComponentProps) => (propertyName, {FieldComponent = DefaultFieldComponent, entityPath, onBlur: userDefinedOnBlur,onChange: userDefinedOnChange,  ...options} = {}, index) => {
        ...
        const rawValidList  = fieldTab.rawValid && fieldTab.rawValid[index] ? !fieldTab.rawValid[index][propertyName] ? false : true : false;
        const validList  = fieldTab.valid && fieldTab.valid[index] ? fieldTab.valid[index][propertyName] == false ? false :  true : true;
        return <FieldComponent {...field}
            textOnly={textOnly}
            editing={editing}
            multiple={multiple}
...
export function connect() {
    return function connectComponent(ComponentToConnect) {
        // TODO : Transform this function into a PureComponent to save at least 1 useless render
